...
const ruRootPageContent = {
  ...
  seoSection: {
    ...
    faqItems: [
      {
        question: "Как добавить таймер-оверлей в видео?",
        answer:
          "Задайте длительность и стиль в генераторе выше, затем экспортируйте. Выберите PNG Sequence для самого надёжного прозрачного таймера-оверлея или WebM для одного прозрачного видеофайла и поместите его на слой поверх вашего видео в CapCut, Premiere Pro, Final Cut Pro, DaVinci Resolve или OBS.",
      },
      {
        question: "Можно ли сделать прозрачный таймер-оверлей для OBS или Twitch?",
        answer:
          "Да. Экспортируйте формат WebM с прозрачностью и добавьте его как видеослой в OBS, Streamlabs или Twitch Studio. Фон уже вырезан, поэтому обратный отсчёт аккуратно ложится поверх сцены без дополнительного хромакея.",
      },
      {
        question:
          "Чем таймер-оверлей отличается от оверлея с обратным отсчётом?",
        answer:
          "Обычно это одно и то же: таймер-оверлей, который ведёт обратный отсчёт от заданной длительности до нуля поверх вашего видео. Этот инструмент создаёт таймер-оверлей с обратным отсчётом, поэтому задайте общее число секунд, выберите формат отображения вроде MM:SS и экспортируйте его для своего редактора.",
      },
      {
